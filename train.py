...
def log_live_dvc(live, params):
    for key in params.keys():
        live.log_metric(key, params[key])
    live.next_step()
...
def main():
    ...
    with Live("/media/fteam4/External/mehdi/background/mehdi/BiRefNet/results") as live:
        for epoch in range(epoch_st, args.epochs+1):
            ...
            # Save checkpoint
            # DDP
            csv_path = os.path.join(args.ckpt_dir, f'results_epoch_{epoch}.csv')
            df.to_csv(csv_path)
            log_live_dvc(live, performance_dict)
...
if __name__ == '__main__':
    ...
    config.lambdas_pix_last = {
        # not 0 means opening this loss
        # original rate -- 1 : 30 : 1.5 : 0.2, bce x 30
        'bce': 30 * 1,          # high performance
        'iou': 0.5 * 1,         # 0 / 255
        'iou_patch': 0.5 * train_params["iou_patch"],   # 0 / 255, win_size = (64, 64)
        'mse': 150 * train_params["mse"],         # can smooth the saliency map
        'triplet': 3 * train_params["triplet"],
        'reg': 100 * train_params["reg"],
        'ssim': 10 * 1,          # help contours,
        'cnt': 5 * train_params["cnt"],          # help contours
        'structure': 5 * train_params["structure"],    # structure loss from codes of MVANet. A little improvement on DIS-TE[1,2,3], a bit more decrease on DIS-TE4.
    }
    main()

# dvc stage add -n train -p lr,optimizer,iou_patch,mse,triplet,reg,cnt,structure -d train.py -m results/metrics.json python train.py
# dvc exp run --name exp --queue -S "lr=0.0001,0.00005,0.00001" -S "optimizer='Adam','AdamW'" -S "iou_patch=0,1" -S "mse=0,1" -S "triplet=0,1" -S "reg=0,1" -S "cnt=0,1" -S "structure=0,1"
# dvc exp run --name exp --queue -S "iou_patch=0,1" -S "mse=0,1" -S "triplet=0,1" -S "reg=0,1" -S "cnt=0,1" -S "structure=0,1"
# dvc exp run --name exp --queue -S "iou_patch=0,1" 
...
# git rm -r --cached 'results/metrics.json'
# git commit -m "stop tracking results/metrics.json" 
# pip uninstall dvc                   
# pip uninstall dvc-data              
# pip uninstall dvc-http              
# pip uninstall dvc-objects           
# pip uninstall dvc-render            
# pip uninstall dvc-studio-client     
# pip uninstall dvc-task              
# pip uninstall dvclive 
# pip install dvclive==3.44.0