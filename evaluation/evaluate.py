...
def evaluate(pred_dir, method, testset, only_S_MAE=False, epoch=0):
    ...
    with open(filename, 'a+') as file_to_write:
        ...
        em, sm, fm, mae, wfm, hce, df = evaluator(
            gt_paths=gt_paths[:],
        ...
        file_to_write.close()
    return {'e_max': e_max, 'e_mean': e_mean, 'e_adp': e_adp, 'sm': sm, 'mae': mae, 'f_max': f_max, 'f_mean': f_mean, 'f_wfm': f_wfm, 'f_adp': f_adp, 'hce': hce}, df
