...
def evaluator(gt_paths, pred_paths, metrics=['S', 'MAE', 'E', 'F', 'WF', 'HCE'], verbose=False):
    ...
    # added by mehdi for debbuging model
    data = {"image_name": [], "SM":[], "MAE":[], "HCE":[]}
    ...
    for idx_sample in tqdm(range(len(gt_paths)), total=len(gt_paths)) if verbose else range(len(gt_paths)):
        ...
        data["image_name"].append(Path(gt).stem)
        data["SM"].append(SM.sms[-1])
        data["MAE"].append(MAE.maes[-1])
        data["HCE"].append(HCE.hces[-1])
    ...
    return em, sm, fm, mae, wfm, hce, pd.DataFrame(data)
